...
def read_seed_outputs(model_size):
    run_dirs = sorted((OUTPUT_DIR / model_size).glob("seed_*"))
    metrics = []
    pearl = []
...
def summarize_model(model_size):
    summary_dir = OUTPUT_DIR / model_size / "summary"
    summary_dir.mkdir(parents=True, exist_ok=True)

    metrics, pearl = read_seed_outputs(model_size)
    pearl_mean = (
        pearl
...
if __name__ == "__main__":
    args = parse_args()
    summarize_model(args.model_size)
